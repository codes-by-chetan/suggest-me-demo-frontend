...
export function ChatPage({ users, currentUserId }: ChatPageProps) {
  ...
  const [conversations] = useState<ChatConversation[]>([
    {
      ...
      lastMessage: {
        id: 'msg1',
        senderId: '2',
        message: 'Have you watched the new Dune movie yet? It\'s incredible! The cinematography is absolutely stunning and the story really builds on the first one.',
    ...
    },
    {
      ...
      lastMessage: {
        ...
        message: 'Thanks for the book recommendation! I really enjoyed it.',
    ...
    },
    {
      ...
      lastMessage: {
        id: 'msg3',
        senderId: '3',
        message: 'Movie night this weekend? We could watch the new releases together!',
    ...
    '4': [
      {
        id: 'msg9',
        senderId: '4',
        message: 'Hey, have you started watching Breaking Bad yet?',
  ...
  const getOtherParticipants = (conversation: ChatConversation) => {
          ...
          <Card className="flex-1 flex flex-col shadow-sm">
            <CardHeader className="pb-3 px-4 md:px-5 border-b">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" className="flex-shrink-0 h-9 w-9 p-0 hover:bg-primary hover:text-primary-foreground transition-colors">
              ...
              <ScrollArea className="h-full">
                <div className="p-2">
                  {filteredConversations.map((conversation) => {
                    const otherUsers = getOtherParticipants(conversation);
                    const isActive = selectedChat === conversation.id;
                          ...
                          <div className="relative flex-shrink-0">
                            {conversation.isGroup ? (
                              ...
                              <Avatar className="w-12 h-12 ring-2 ring-background">
                                <AvatarImage src={getChatAvatar(conversation)} />
                                <AvatarFallback className="bg-gradient-to-br from-primary/20 to-primary/10 text-primary">
                                  {otherUsers[0]?.displayName.charAt(0)}
                                </AvatarFallback>
                              </Avatar>
                            )}
                            {hasUnread && (
                              <Badge className="absolute -top-1 -right-1 h-5 min-w-[20px] px-1.5 flex items-center justify-center text-xs shadow-sm">
                                {conversation.unreadCount > 9 ? '9+' : conversation.unreadCount}
                              </Badge>
                            )}
                            {!hasUnread && !conversation.isGroup && (
                              <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-background" />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-baseline justify-between gap-2 mb-0.5">
                              <h3 className={`truncate flex-1 min-w-0 ${hasUnread ? 'font-semibold' : 'font-medium'}`}>
                            ...
                            <div className="flex items-center gap-1.5">
                              <p className={`text-sm truncate flex-1 min-w-0 ${
                                ...
                                {conversation.lastMessage.type === 'recommendation' 
              ...
              <CardHeader className="pb-3 px-4 md:px-5 flex-shrink-0 border-b">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    {selectedConversation.isGroup ? (
                      ...
                      <Avatar className="w-10 h-10 flex-shrink-0">
                        <AvatarImage src={getChatAvatar(selectedConversation)} />
                        <AvatarFallback className="bg-gradient-to-br from-primary/20 to-primary/10 text-primary">
                          {getOtherParticipants(selectedConversation)[0]?.displayName.charAt(0)}
                        </AvatarFallback>
                      </Avatar>
                    )}
                    <div className="flex-1 min-w-0">
                      <h2 className="font-semibold truncate">{getChatName(selectedConversation)}</h2>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hidden sm:flex hover:bg-muted">
                    ...
                    {chatMessages.map((message, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`max-w-[85%] md:max-w-[70%] ${isOwn ? 'order-first' : ''}`}>
                            {message.type === 'system' ? (
                              ...
                              <div className={`p-3 rounded-2xl border-2 shadow-sm ${
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  <Star className={`w-4 h-4 flex-shrink-0 ${isOwn ? 'text-primary-foreground' : 'text-primary'}`} />
                                  <span className="text-sm font-medium truncate">Recommended content</span>
                                </div>
                                <div className="flex items-center gap-3 p-2.5 bg-background/10 backdrop-blur-sm rounded-lg">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium truncate">Dune: Part Two</p>
                                    <p className="text-xs opacity-70 truncate">2024 • Sci-Fi</p>
                                  </div>
                                  <Button 
                                    size="sm" 
                              ...
                              <div className={`group relative px-4 py-2.5 rounded-2xl break-words shadow-sm ${
                                isOwn 
                  ...
                  <div className="flex-1 relative min-w-0">
                    ...
                    {showEmojiPicker && (
                      <>
                        ...
                        />
                        {/* Emoji Picker Panel */}
                        <div className="absolute bottom-full right-0 mb-2 z-50 w-[min(340px,calc(100vw-2rem))]" onClick={(e) => e.stopPropagation()}>
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex gap-1 p-2 border-b overflow-x-auto scrollbar-hide">
        {Object.keys(emojiCategories).map((category) => (
          <Button
            ...
            className="text-xs px-3 h-8 flex-shrink-0 rounded-full"
          >
            {category}
          </Button>
        ))}
      ...
      <ScrollArea className="h-[280px]">
        <div className="grid grid-cols-8 gap-0.5 p-2">
          {displayEmojis.map((emoji, index) => (
            <button
              key={index}
              className="text-2xl p-2 hover:bg-accent rounded-lg transition-all active:scale-95 transform hover:shadow-sm"
      ...
      <div className="p-3 border-t bg-muted/30">
        <p className="text-xs text-muted-foreground mb-2 px-1 font-medium">Frequently Used</p>
        <div className="flex gap-1">
          {['😊', '👍', '❤️', '😂', '🎉', '🔥', '✨', '💯'].map((emoji, index) => (
            <button
              key={index}
              className="text-xl p-2 hover:bg-accent rounded-lg transition-all active:scale-95 transform flex-1 hover:shadow-sm"
              onClick={() => onEmojiSelect(emoji)}
            >
              {emoji}
            </button>
